...
/* Parallelised version of Maurice Gilden's Go port of Kevin Beason's Monte Carlo Path Tracer
*/
...
func piece(y, n int, cx Vec, cy Vec, c chan int) {
	for ; y < n; y++ {
		for x := 0; x < w; x++ {
...
func main() {
	const NCPU = 8  //todo get actual number of cores
	...
	c := make(chan int, NCPU)  //buffered
	for i := 0; i < NCPU; i++ {
		fmt.Printf("Rendering (%d spp) (chunk %d from %d to %d)\n",samps*4,i,i*h/NCPU,(i+1)*h/NCPU);
		go piece(i*h/NCPU, (i+1)*h/NCPU, cx, cy, c)
	}
	// Drain the channel.
	for i := 0; i < NCPU; i++ {
		<-c    // wait for one task to complete
	}
	f, _ := os.Create("image_go.ppm")
